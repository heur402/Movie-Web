...
import {
  Play, Heart, Star, Share2, Download,
...
const MovieWatch = () => {
  ...
  const handleShare = () => {
    ...
    else { navigator.clipboard.writeText(window.location.href); alert(t("movie.link_copied")); }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-semibold text-gray-300 uppercase tracking-wider">{t("movie.trailer")}</span>
              </div>
              <div className="relative bg-black rounded-2xl overflow-hidden shadow-2xl aspect-video">
                {embedUrl ? (
                  <iframe src={embedUrl} title={`${movie.title} — Trailer`}
                    className="w-full h-full"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen />
                ) : movie.trailer ? (
                  <video src={movie.trailer} controls muted autoPlay className="w-full h-full" poster={poster} />
                ) : (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-gray-900/80 gap-4">
                    {poster && <img src={poster} alt={movie.title} className="absolute inset-0 w-full h-full object-cover opacity-15" />}
                    <div className="relative z-10 text-center px-6">
                      <Film size={48} className="text-gray-600 mx-auto mb-3" />
                      <p className="text-gray-400 font-medium">{t("movie.no_trailer")}</p>
                      <p className="text-gray-600 text-sm mt-1">{t("movie.no_trailer_sub")}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              ...
              <div className="flex items-center gap-2 mt-4 flex-wrap">
                {hasFullMovie ? (
                  <Link to={`/watch/${id}`}
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white text-sm font-bold transition-all shadow-lg shadow-red-600/30">
                    <Play size={16} fill="white" /> {t("movie.watch_full_movie")}
                  </Link>
                ) : (
                  <span className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gray-800 border border-gray-700 text-gray-500 text-sm font-medium cursor-not-allowed">
                    <Film size={16} /> {t("movie.full_movie_unavailable")}
                  </span>
                )}
                ...
                <button onClick={() => toggleFavorite(movie)}
                  ...
                  {isFavorite(movie._id) ? t("movie.saved") : t("movie.save")}
                </button>

                <button onClick={handleShare}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/10 bg-white/5 text-gray-300 hover:bg-white/10 text-sm font-medium transition-all">
                  <Share2 size={15} /> {t("movie.share")}
                </button>
                ...
                {hasFullMovie && (
                  <button
                    onClick={(e) => {
                      ...
                      a.href = url; a.download = movie.title || "movie"; a.target = "_blank";
                      document.body.appendChild(a); a.click(); document.body.removeChild(a);
                    }}
                    className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/10 bg-white/5 text-gray-300 hover:bg-white/10 text-sm font-medium transition-all">
                    <Download size={15} /> {t("movie.download")}
                  </button>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 mt-5 p-4 bg-white/5 rounded-xl border border-white/10">
                ...
                <div className="text-sm">
                  ...
                  <span className="text-gray-400 ml-1">{t("movie.rating_label")}</span>
                  {movie.ratings?.length > 0 && (
                    <span className="text-gray-500 ml-2">{t("movie.ratings_count", { count: movie.ratings.length })}</span>
                  )}
                </div>
                {userRating > 0 && (
                  <span className="text-green-400 text-xs ml-auto">
                    {t("movie.rated_confirmation", { score: userRating })}
                  </span>
                )}
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              ...
              <form onSubmit={handleComment} className="space-y-3 mb-6">
                ...
                <div className="flex gap-2">
                  <textarea value={commentText} onChange={(e) => setCommentText(e.target.value)}
                    placeholder={t("comments.text_placeholder")} rows={2}
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto scrollbar-thin">
                {comments.length === 0
                  ? <p className="text-gray-500 text-sm text-center py-4">{t("movie.no_comments")}</p>
                  : comments.map((c, i) => (
          ...
          <div className="lg:col-span-1">
            ...
            {relatedMovies.length === 0
              ? <p className="text-gray-500 text-sm">{t("movie.no_related")}</p>
              : (
                <>
                  {/* Mobile: horizontal */}
                  <div ref={relatedRef} className="lg:hidden flex gap-3 overflow-x-auto pb-2 scroll-smooth scrollbar-hide">
                    {relatedMovies.map((rel, i) => (
                      <div key={rel._id} className="shrink-0 w-36"><MovieCard movie={rel} index={i} /></div>
                    ))}
                  </div>
                    ...
                    {relatedMovies.map((rel) => (
                      <Link key={rel._id} to={`/movie/${rel._id}`}
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-white text-sm font-semibold truncate group-hover:text-red-400 transition-colors">{rel.title}</h3>
