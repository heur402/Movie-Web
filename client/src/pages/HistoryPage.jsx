      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-24 pb-16">
          ...
          <div>
            ...
            <p className="text-gray-400">
              {watchHistory.length > 0
                ? t("history.in_progress_count", { count: watchHistory.length, plural: watchHistory.length !== 1 ? "s" : "" })
                : t("history.empty_label")}
            </p>
          </div>
          {watchHistory.length > 0 && (
            <button
        ...
        {watchHistory.length === 0 ? (
          ...
          >
            ...
            <h3 className="text-xl font-bold mb-2">{t("history.empty_title")}</h3>
            <p className="text-gray-400 text-sm mb-6">{t("history.empty_sub")}</p>
            <Link to="/" className="px-6 py-3 bg-red-600 hover:bg-red-500 text-white rounded-xl font-semibold transition-colors">
              {t("history.browse_movies")}
            </Link>
          </motion.div>
        ) : (
              ...
              >
                <div className="relative aspect-video overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <Link to={`/movie/${movie._id}`} className="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center shadow-xl">
                      <Play size={20} fill="white" className="text-white ml-0.5" />
                    </Link>
                  </div>
                  {movie.progress > 0 && (
                    <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/20">
                      <div className="h-full bg-red-500 transition-all" style={{ width: `${Math.min(movie.progress, 100)}%` }} />
                    </div>
                  )}
                </div>
                <div className="p-3">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    ...
                    {movie.progress > 0 && (
                      <span className="text-gray-500 text-xs">
                        {t("history.percent_watched", { percent: movie.progress })}
                      </span>
                    )}
                  </div>
                  <Link
                  ...
                  >
                    <Play size={12} /> {t("history.continue_watching")}
                  </Link>
                </div>
              </motion.div>
            ))}
